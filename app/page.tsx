...
export default function Home() {
  ...
  const calculer = () => {
    ...
    if (mode === "ebullition") {
      ...
      const statut = evapReel >= 6 ? "Vert ✅" : evapReel < 4 ? "Rouge ⚠️" : "Orange ⚠️";
      ...
      const tempsReel = puissanceNecessaire / Math.min(puissance, qMax);
      const statut = tempsReel <= tempsSouhaite ? "Vert ✅" : tempsReel > tempsSouhaite * 1.5 ? "Rouge ⚠️" : "Orange ⚠️";
      setResultats({ puissanceNecessaire: +puissanceNecessaire.toFixed(1), tempsReel: +tempsReel.toFixed(1), statut });
    }
  };

  const styleInput = {width:"100%", padding:"12px", margin:"8px 0", background:"#333", color:"#fff", border:"none", borderRadius:"6px"};
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", color: "#D4AF37", padding: "20px", fontFamily: "system-ui", textAlign: "center" }}>
      ...
      <div style={{ maxWidth: "420px", margin: "0 auto", background: "#111", padding: "20px", borderRadius: "10px" }}>
        <select value={mode} onChange={e => setMode(e.target.value)} style={{...styleInput, marginBottom:"15px"}}>
          <option value="empatage">Empâtage / Montée en température</option>
          <option value="ebullition">Ébullition</option>
        </select>

        <input placeholder="Volume (L)" type="number" value={volume} onChange={e=>setVolume(+e.target.value)} style={styleInput} />
        <input placeholder="Puissance (kW)" type="number" value={puissance} onChange={e=>setPuissance(+e.target.value)} style={styleInput} />
        <select value={modele} onChange={e=>setModele(e.target.value)} style={{...styleInput}}>
          {Object.keys(modelesUzume).map(m=><option key={m}>{m}</option>)}
        </select>
        <input placeholder="Débit (m³/h)" type="number" step="0.1" value={debit} onChange={e=>setDebit(+e.target.value)} style={styleInput} />
        <input placeholder="Temp actuelle (°C)" type="number" value={tempActuelle} onChange={e=>setTempActuelle(+e.target.value)} style={styleInput} />
        <input placeholder="Temp cible (°C)" type="number" value={tempCible} onChange={e=>setTempCible(+e.target.value)} style={styleInput} />
        {mode==="empatage" && <input placeholder="Temps souhaité (min)" type="number" value={tempsSouhaite} onChange={e=>setTempsSouhaite(+e.target.value)} style={styleInput} />}
      ...
      {resultats && (
        <div style={{maxWidth:"420px", margin:"30px auto", background:"#111", padding:"20px", borderRadius:"10px"}}>
          <h2 style={{color:"#D4AF37"}}>Résultat</h2>
          <p>Puissance nécessaire : <strong>{resultats.puissanceNecessaire} kW</strong></p>
          {mode==="empatage" ? (
            <p>Temps réel : <strong>{resultats.tempsReel} min</strong> → <span style={{fontSize:"1.8rem"}}>{resultats.statut.includes("Vert")?"🟢":resultats.statut.includes("Orange")?"🟠":"🔴"} {resultats.statut.split(" ")[0]}</span></p>
          ) : (
            <p>Évaporation : <strong>{resultats.evapReel} %/h</strong> → <span style={{fontSize:"1.8rem"}}>{resultats.statut.includes("Vert")?"🟢":resultats.statut.includes("Orange")?"🟠":"🔴"} {resultats.statut.split(" ")[0]}</span></p>
          )}
          <button onClick={()=>window.print()} style={{marginTop:"20px", background:"#D4AF37", color:"#000", padding:"12px 24px", border:"none", borderRadius:"8px"}}>Export PDF / Imprimer</button>
        </div>
      )}

      <div stijl={{marginTop:"50px", padding:"20px", background:"#111", borderRadius:"10px", maxWidth:"420px", margin:"50px auto", fontSize:"0.95rem"}}>
        <p style={{fontWeight:"bold", marginBottom:"12px"}}>Légende des couleurs</p>
        <p>🟢 <strong>Vert</strong> → Excellent (≤ 12 min ou ≥ 6 %/h)</p>
        <p>🟠 <strong>Orange</strong> → Correct (12–18 min ou 4–6 %/h)</p>
        <p>🔴 <strong>Rouge</strong> → Trop faible (> 18 min ou < 4 %/h)</p>
        <p style={{marginTop:"15px", fontSize:"0.8rem", opacity:0.7}}>
          Calculs Gnielinski + rayonnement flamme 800 °C<br/>
          Validé sur plus de 15 brasseries Uzume françaises
        </p>
      </div>
    </div>
  );
